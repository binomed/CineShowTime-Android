		...
		, OnItemClickListener //
{
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		Intent intentCleanFileService = new Intent(mainContext, CineShowCleanFileService.class);
		mainContext.startService(intentCleanFileService);
	...
	private void initViews(View mainView) {
		...
		buttonSearchNear = (Button) mainView.findViewById(R.id.mainBtnSearchNear);
	...
	private void display() {
		if (this.showLastChange()) {
			LastChangeDialog dialog = new LastChangeDialog(mainContext);
			dialog.show();
		}
	...
	public void openSearchActivity(TheaterBean theater) {
		Intent intentStartNearActivity = new Intent(mainContext, CineShowTimeSearchActivity.class);
	...
	public boolean showLastChange() {
		...
		if (mDbHelper.isOpen()) {
			...
			try {
				PackageInfo pi = mainContext.getPackageManager().getPackageInfo(mainContext.getPackageName(), 0);
		...
		if (result) {
			Intent intentService = new Intent(mainContext, CineShowDBGlobalService.class);
			...
			mainContext.startService(intentService);
		}
